"""
This module contains the datasets that are directly usable in the experiments.
...
class Dataset_RGBProMP(data.DatasetOrdinary, data.DatasetRGB):
        ...
        def __init__(self, dataset_dir, rgb_dir,depth_dir, **kwargs):
            super().__init__(dataset_dir=dataset_dir, rgb_dir=rgb_dir, depth_dir=depth_dir, **kwargs)

        def prepare_data(self, val_frac=0.0, N_test=0, random_state=None,use_val_in_train=False):
            idx_train, idx_val, idx_test = self._split_train_test_val(val_frac, N_test, random_state,use_val_in_train)
            n_train = len(idx_train)
            n_test = len(idx_test)
            n_val = len(idx_val)
            X_train, y_train = {"RGB":self.samples["img_enc"][idx_train].astype('float64'),"D":self.samples["img_enc_D"][idx_train].astype('float64')}, \
            ...
            X_val, y_val = {"RGB":self.samples["img_enc"][idx_val].astype('float64'),"D":self.samples["img_enc_D"][idx_val].astype('float64')}, \
            ...
            X_test, y_test = {"RGB":self.samples["img_enc"][idx_test].astype('float64'),"D":self.samples["img_enc_D"][idx_test].astype('float64')}, \
