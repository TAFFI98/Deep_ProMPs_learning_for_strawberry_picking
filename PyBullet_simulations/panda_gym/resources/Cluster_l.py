...
class Cluster:
    def __init__(self, cluster_pose, client_id):
        ...
        for i in range(self.n_long_red_berries):
            ...
            if r == 0 or r == 1:
                ...
                y_noise = 0
                self.app_angle_min = -0.1
                self.app_angle_max = -0.55
            elif r == 6 or r == 7:
                y_noise = -0.05
                self.app_angle_min = -0.1
                self.app_angle_max = -0.6
            elif r == 8 or r == 9:
                y_noise = 0.05
                self.app_angle_min =  -0.1
            ...
            berry_pose_g = berry_pose
            berry_pose_g[0] -= 0.05
            berry_pose_g[1] -= 0.03
            self.green_strawberry = Strawberry.Strawberry(client_id=client_id, start_pose=berry_pose_g, start_ori=berry_or, red = False,ShortMedLong=[1,0,0])
            ...
            for i in range(0, p.getNumJoints(self.green_strawberry.strawberry)):
                p.changeDynamics(self.green_strawberry.strawberry, i, lateralFriction=0, spinningFriction=0,
                                 rollingFriction=0, restitution=0, contactStiffness=0.0, contactDamping=0.00000,
                                 linearDamping=0.0000, angularDamping=0.00000)
        ...
        for i in range(self.n_short_red_berries):
            ...
            berry_pose_g[0] -= 0.05
            berry_pose_g[1] -= 0.03
            self.green_strawberry = Strawberry.Strawberry(client_id=client_id, start_pose=berry_pose_g, start_ori=berry_or, red = False,ShortMedLong=[0,1,0])
    ...
    def disable_collisions(self,client_id,panda):
        r_n_berry = p.getNumJoints(self.red_strawberry.strawberry,physicsClientId=client_id)-1
        link_r_berry= np.linspace(0,p.getNumJoints(self.red_strawberry.strawberry,physicsClientId=client_id)-1,num=r_n_berry+1)
        g_n_berry = p.getNumJoints(self.green_strawberry.strawberry,physicsClientId=client_id)-1
        link_g_berry= np.linspace(0,p.getNumJoints(self.green_strawberry.strawberry,physicsClientId=client_id)-1,num=g_n_berry+1)
        for i in range(r_n_berry):
            p.setCollisionFilterPair(panda,self.red_strawberry.strawberry,10,int(link_r_berry[i]),0)
            p.setCollisionFilterPair(panda,self.red_strawberry.strawberry,11,int(link_r_berry[i]),0)
            p.setCollisionFilterPair(panda,self.red_strawberry.strawberry,9,int(link_r_berry[i]),0)
            p.setCollisionFilterPair(panda,self.red_strawberry.strawberry,8,int(link_r_berry[i]),0)
            p.setCollisionFilterPair(panda,self.red_strawberry.strawberry,7,int(link_r_berry[i]),0)
            p.setCollisionFilterPair(panda,self.red_strawberry.strawberry,6,int(link_r_berry[i]),0)
        for i in range(g_n_berry):
            p.setCollisionFilterPair(panda,self.green_strawberry.strawberry,10,int(link_g_berry[i]),0)
            p.setCollisionFilterPair(panda,self.green_strawberry.strawberry,11,int(link_g_berry[i]),0)
            p.setCollisionFilterPair(panda,self.green_strawberry.strawberry,9,int(link_g_berry[i]),0)
            p.setCollisionFilterPair(panda, self.green_strawberry.strawberry,8, int(link_g_berry[i]), 0)
            p.setCollisionFilterPair(panda,self.green_strawberry.strawberry,7,int(link_g_berry[i]),0)
            p.setCollisionFilterPair(panda,self.green_strawberry.strawberry,6,int(link_g_berry[i]),0)
